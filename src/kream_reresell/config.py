...
class Settings:
    ...
    api_budget_per_10min: int = field(default_factory=lambda: _int("API_BUDGET_PER_10MIN", pacing.DEFAULT_LIMIT))
    # 접속 예산 - 10분 창 안의 페이지 이동을 이만큼까지만 (pacing 대응 5). 0 이면 안 센다
    page_budget_per_10min: int = field(default_factory=lambda: _int("PAGE_BUDGET_PER_10MIN", pacing.DEFAULT_PAGE_LIMIT))
