"""랭킹 탭에서 랭킹을 골라 순위대로 상품 ID 를 모은다."""
...
# 랭킹 탭의 랭킹 칩 순서 그대로 (2026-09-04 실측). 값은 URL 의 category_filter.
...
class RankedProduct:
    ...
    url: str
    category: str = ""
    trades: int | None = None   # 카드에 적힌 전체 거래 수 ("거래 4.1만"). 모르면 None. 기준 미달이면 상품 페이지를 열지 않는다 (app)


_COUNT = re.compile(r"(\d[\d,]*(?:\.\d+)?)\s*(만)?")


def parse_count(text: str | None) -> int | None:
    """카드의 '거래 2.3만' / '거래 1,494' 를 정수로. 없으면 None. ('만' 은 반올림된 값이라 15건 근처에서는 항상 정확한 숫자다)"""
    if not text:
        return None
    m = _COUNT.search(text.replace("거래", ""))
    if not m:
        return None
    n = float(m.group(1).replace(",", ""))
    return int(round(n * 10_000)) if m.group(2) else int(n)
...
def open_category(page: Page, category: str) -> None:
    ...
    알고 있는 category_filter 로 바로 열고, 선택된 칩이 다른 랭킹이면(ID 가 바뀐 경우)
    ...
    if chip.count() == 0:
        raise ValueError(f"랭킹 탭에 '{category}' 칩이 없습니다")
    ...
    if m:
        ...
        log.info("랭킹 '%s' -> category_filter=%s", category, m.group(1))
    page.locator('a[href*="/products/"]').first.wait_for(state="visible", timeout=15_000)
    page.wait_for_timeout(800)   # 이전 랭킹 카드가 새 카드로 바뀔 시간
    ...
    while len(seen) < limit and stale_rounds < 4:
        ...
        for item in page.evaluate(_COLLECT_JS):
            ...
            seen[pid] = RankedProduct(
                ...
                category=category,
                trades=parse_count(item.get("trades")),
            )
        if len(seen) == before:
            stale_rounds += 1
        else:
...
() => {
  ...
  for (const a of document.querySelectorAll('a[href*="/products/"]')) {
    ...
    const trades = lines.find(s => /^거래\\s/.test(s)) || null;
