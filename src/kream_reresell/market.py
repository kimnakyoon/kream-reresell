...
GET api.kream.co.kr/api/p/products/{상품ID}?base_product_id={상품ID} 응답의 sales_options[] 에 옵션마다
  ...
  - lowest_95                   95점 보관 판매 최저가 (모달의 '95점' 줄 - 우리는 안 씀)
  - lowest_normal               일반배송 최저가 (= 즉시 구매가. A 보다 쌀 수도 있다 - A 는 반드시 lowest_100)
...
market.total_sales 는 상품 전체 거래 수(기간·배송 구분 없음)라 30일 빠른배송 건수 판정에는 못 쓴다 - 거래량은 지금처럼 체결 내역 패널로 센다.
...
class OptionPrice:
    ...
    fast95: int | None          # 95점 빠른배송 최저가
    normal: int | None          # 일반배송 최저가
...
def parse_market(product_id: int, body: dict) -> ProductMarket:
    ...
    for entry in raw_options:
        ...
        if not key and not label:
            continue
        market.options.append(OptionPrice(
            key=key or label, label=label or key,
            fast=_amount(entry.get("lowest_100")), fast95=_amount(entry.get("lowest_95")),
            normal=_amount(entry.get("lowest_normal")), sell=_amount(entry.get("highest_bid")),
        ))
    m = body.get("market") or {}
    market.total_sales = _amount(m.get("total_sales"))
    return market
...
def fetch_market(client: ApiClient, product_id: int) -> ProductMarket:
    ...
    log.info("시세 API: 상품 %d 옵션 %d개 - %s", product_id, len(market.options),
             ", ".join(f"{o.label} A={_won(o.fast)} B={_won(o.sell)}" for o in market.options[:12])
