...
**B = 내가 1순위가 되는 입찰가** (price_b, 사용자 결정 2026-09-13): 즉시 판매가 + BID_STEP(1,000원). 즉시 판매가 그대로 입찰하면 같은 금액으로
먼저 넣은 남의 입찰 뒤에 붙어 언제 체결될지 모르고, 1,000원을 얹으면 내가 1순위가 된다. 이미 넣은 내 입찰(희망가 my_price)이 밀리지 않았으면
(즉시 판매가 <= 내 희망가 - 살아 있는 입찰이면 같은 값) 이미 1순위라 B = 내 희망가 그대로다 - 여기에도 얹으면 입찰 때 통과한 마진이
재입찰·입찰취소 때 1,000원 모자라 지웠다가 [입찰] 이 다시 넣는 일이 반복된다. 마진 판정(S − B, S = min(A, R) - 2026-09-17 부터, product 머리글)과 입찰가 모두 이 B 를 쓴다
([입찰] 의 시세 거르기 · 구매 페이지 재판정, [재입찰]·[입찰취소] 의 다시 판정). 시세 API 든 구매 페이지든 즉시 판매가를 읽은 곳은 원값(sell)만 주고
B 는 price_b 한 곳에서만 만든다.
...
BID_STEP = 1000   # 즉시 판매가에 얹어 1순위가 되는 금액 (머리글)


def price_b(sell: int | None, my_price: int | None = None) -> int | None:
    """B = 내가 1순위가 되는 입찰가 (머리글). 즉시 판매가 sell + BID_STEP. 이미 넣은 내 입찰(my_price)이 밀리지 않았으면(sell <= my_price)
    내 희망가 그대로. 새 입찰은 my_price 없이 부른다. 즉시 판매가가 없으면 None."""
    if sell is None:
        return None
    return my_price if my_price and sell <= my_price else sell + BID_STEP


def describe_b(sell: int, b: int) -> str:
    """로그용: '즉시 판매가 11,000원 → B 12,000원'."""
    return f"즉시 판매가 {sell:,}원 → B {b:,}원"
